...
export default class CoalescerListView extends ItemView {
    ...
    public readonly redraw = (): void => {
        ...
        let filt:[string,number][] = [];

        Object.entries(this.app.metadataCache.unresolvedLinks).forEach(([key, value]) => {
            if(!this.plugin.state.omittedPaths.every(matcher => {
                console.log(`!key.match(new RegExp(matcher,'i'))`, !key.match(new RegExp(matcher,'i')), '  :  ',matcher,'  :  ', key, value)
                return !key.match(new RegExp(matcher,'i'))
            })) return;
			Object.entries(value).forEach(([k, v]) => filt.push([k,v]))}
		);
        console.log('ERWERWERWER',filt.length)
        filt = [...new Set(filt)];
        console.log(filt.length)
        filt.forEach(([key,value]) => this.items.push([new UnresolvedLink(key,value),false]));
