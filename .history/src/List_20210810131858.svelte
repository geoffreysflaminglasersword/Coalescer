...
<div class="listToggles">
  ...
  <div class="checkbox-container disabled {showFiles ? 'is-enabled' : ''}" on:click={() => (showFiles = !showFiles)} />
  <div class="toggleText">Show Tags</div>
  <div class="checkbox-container disabled {showTags ? 'is-enabled' : ''}" on:click={() => (showTags = !showTags)} />
</div>
<span>Files selected: {numFilesSelected}  Tags selected: {numTagsSelected}</span>
</div>
