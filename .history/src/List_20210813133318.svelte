    ...
    */
    ...
    if (pivot instanceof Tag) {
      ...
      callback = (file, contents, find) => {
        ...
        if (!find.find(item => {console.log('INST: ',item instanceof File, item.path == file.path) return item instanceof File && item.path == file.path;})) return true;
    ...
    ((event: MouseEvent) => {
      ...
      if (numSelected) {
        ...
        selected.forEach((item) => {
          ...
          console.log(resultLim, items, unselected);
