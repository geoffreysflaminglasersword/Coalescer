<script lang="typescript">
    ...
    */
    ...
    if (pivot instanceof Tag) {
      ...
      callback = (file, contents, find) => {
        ...
        console.log(`!find.find((item) => item instanceof File && item.path == file.path
          )`, find.find((item) => item instanceof File && item.path == file.path
          ))
        if (
          find.find((item) => {
            console.log("INST: ", item instanceof File, (item as File)?.path, file.path, (item as File)?.path == file.path,item instanceof File && item.path == file.path);
            return item instanceof File && item.path == file.path;
          })
        ) {
          return true;
        }
  ...
  async function findAndReplaceWith(find: Item | Item[], replace: Item, callback?: CB) {
