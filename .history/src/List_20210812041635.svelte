<script lang="ts">
  ...
  import { HasCount, Item, Tag, File, Link, CoalescerState, linkOpts } from "./Settings";
  ...
  function coalesceTags() {
    /* 
      ...
      if we select a tag first:
        ...
        replace all other tags/links with that tag

      if we select a file first:
        if we have files, merge their contents and delte them
        if we have links/tags, replace them all with links to the file
