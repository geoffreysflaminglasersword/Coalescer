<script lang="ts">
  ...
  import { App, getAllTags, Menu, parseFrontMatterTags, WorkspaceLeaf } from "obsidian";
  ...
  function coalesceTags() {
    ...
    */
    ...
    let filt = items.filter(([item,active]) => item != pivot  && active).map(([item]) => item.basename);
    findAndReplaceWith(pivot.basename,filt);
  }
  ...
  function findAndReplaceWith(replace:string, find: string|string[]){
    if(Array.isArray(find)) for(let i of find) findAndReplaceWith(replace,i);
      ...
      files.forEach(file => {
        ...
        let rep = getAllTags(cache)?.find(tag => {
          return tag == find;
        });
        let rep2 = cache.tags?.find(tc => tc.tag == '#'+find);
        ;
        console.log(`\n`, '\n');
        if(getAllTags(cache)) getAllTags(cache).forEach(tc => console.log(`\t\t\tcache.tags`,tc.tag ))
        if(rep || rep2){
