...
export function getYamlProp(propName: YAMLProp, source: string) {
    ...
    console.log(`val`, val,typeof val,match)
    return val?.match(/\[/) ? val.replace('[', '').replace(']', '').split(', ') : val;
}

//NOTE: if this presents a performance issue, convert to a generator method
...
export function subsets<T>(arr:T[]) : T[][] {
    return arr.reduce((subsets:any[], value) => subsets.concat(subsets.map(set => [value,...set])),[[]]);
}
