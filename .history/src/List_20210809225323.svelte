  ...
  let click = (item: Item) =>
    ((event: MouseEvent) => {
      if (numSelected == 1 && prevNumSelected == 0) {
        ...
        console.log(`filt`, filt, matchSorter(filt, pivot.path, { keys: ["*.count","*.path"] }));
        ...
        items = [
          ...new Set<[Item, boolean]>([
            ...
            ...matchSorter(filt, pivot.path.slice(0,-1), { keys: ["*.path"] }),
            ...matchSorter(filt, pivot.path.slice(1), { keys: ["*.path"] }),
            ...matchSorter(filt, pivot.path.slice(0,-3), { keys: ["*.path"] }),
            ...matchSorter(filt, pivot.path.slice(3), { keys: ["*.path"] }),
            ...matchSorter(filt, pivot.path.slice(0,-5), { keys: ["*.path"] }),
            ...matchSorter(filt, pivot.path.slice(5), { keys: ["*.path"] }),
            ...matchSorter(filt, pivot.path.slice(0,-8), { keys: ["*.path"] }),
            ...matchSorter(filt, pivot.path.slice(8), { keys: ["*.path"] }),
            ...filt,
          ]),
        ];
