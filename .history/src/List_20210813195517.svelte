<script lang="typescript">
  ...
  function coalesceMenuCreate() {
    ...
    if (pivot instanceof File) {
      ...
      menu.addItem((item) => {
        item.setTitle("Create Standard Aliases").onClick(async (e) => {
          items.forEach((val) => (val.selected = false));
          reset();
        });
      });
    ...
    if (!Array.isArray(find)) findAndReplaceWith([find], replace, useAlias, preprocess);
      ...
      let replaceTagsLinksAndEmbeds = async (file: TFile, contents: string) => {
        ...
        foundAlts.forEach((alt) => {
          if (useAlias && !(alt instanceof Tag)){console.log('HERE',alt); aliases.push(alt.basename);}
          contents = contents.replace(alt.altRepresentation, pivot.altRepresentation);
        });
        ...
        // console.log(`fmtags,filtered`, fmtags, filtered);
      ...
      for (let file of app.vault.getMarkdownFiles()) {
        replaceTagsLinksAndEmbeds(file, preprocess?.(file, await app.vault.cachedRead(file), find));
      }
      if (pivot instanceof File) {
        let contents = await app.vault.cachedRead(pivot.tfile);
        console.log(`aliases`, aliases)
        if (aliases.length) contents = updateYamlProp("aliases", aliases, contents);
        replaceTagsLinksAndEmbeds(pivot.tfile, contents);
