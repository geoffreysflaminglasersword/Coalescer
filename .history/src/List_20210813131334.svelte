<script lang="ts">
  ...
  let selected = new Set<Item>(),
    numSelected = 0, lastSuggestion = 0;
    ...
    ((event: MouseEvent) => {
      ...
      if (numSelected) {
        ...
        selected.forEach((item) => {
          unselected.sort((a, b) =>
          sim.compareTwoStrings(a.basename, item.basename) > sim.compareTwoStrings(b.basename, item.basename) ? -1 : 1
          );
          console.log(item == pivot ? "Pivot" : "Not",unselected);
          let resultLim: number;
          for (resultLim = 15; resultLim > 0; resultLim--)
          if (sim.compareTwoStrings(item.basename, unselected[resultLim].basename)) break;
