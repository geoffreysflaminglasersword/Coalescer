<script lang="ts">
  ...
  import { App, CachedMetadata, getAllTags, Menu, parseFrontMatterTags, WorkspaceLeaf } from "obsidian";
  ...
  function coalesceMenuCreate() {
    ...
    menu.addItem((item) => {
      item.setTitle("Coalesce Selected").onClick((e) => {
        ...
        if (numFilesSelected || numLinksSelected) coalesceToLinks();
  ...
  function coalesceTags() {
    ...
    */
    ...
    findAndReplaceWith(activeItems, pivot);
  }

  function findAndReplaceWith( find: Item|Item[],replace:Item,){
    if(!Array.isArray(find)) findAndReplaceWith([find],replace);
    else{
      console.log('Here 1')


      const itemStrat = (item:Item, tags: Set<string>,links: Set<string>) => {
        if(item instanceof Tag) return tags.has(item.altRepresentation);
        else return links.has(item.basename);
      }
      
      let files = app.vault.getMarkdownFiles();
      console.log(`lc`,replace, find);
      files.forEach(file => {
        let cache = app.metadataCache.getFileCache(file);
        let tags = new Set(getAllTags(cache));
        let links = new Set(cache.links?.map(lc => lc.link));
        let foundAlts = find.filter(val => itemStrat(val,tags,links)).map(i => i.altRepresentation);
        console.log('Here')
        if(foundAlts.length) console.log(`foundAlts`, file,'\n', tags,'\n',links,'\n',foundAlts)
        // if(tags) console.log(`tags`, tags);
        if(cache.links?.find(l => l.original.match(/homer/i))) console.log(`links`, cache.links)
        if(cache.embeds?.find(l => l.original.match(/homer/i))) console.log(`embeds`, cache.embeds)
