...
export default class CoalescerListView extends ItemView {
    ...
    public readonly redraw = (): void => {
        ...
        let unresolved = {};
        let resolve = {};
        Object.entries(this.app.metadataCache.unresolvedLinks).filter(([key,val]) => Object.entries(val)?.length).forEach(([key,val]) => unresolved[key] = val);
        Object.entries(this.app.metadataCache.resolvedLinks).filter(([key,val]) => Object.entries(val)?.length).forEach(([key,val]) => resolve[key] = val);
        console.log(`this.app.metadataCache.unresolvedLinks`,unresolved)
        console.log(`this.app.metadataCache.resolvedLinks`,resolve)
        ...
        let allLinks = Object.entries(Object.assign(this.app.metadataCache.unresolvedLinks,this.app.metadataCache.resolvedLinks))
        allLinks.forEach(([path, link]) => {
