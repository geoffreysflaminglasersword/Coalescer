<script lang="ts">
  ...
  let selected = new  Set<Item>(), numSelected:number = 0;


  let click = (item: Item) =>
    ((event: MouseEvent) => {
      console.log(`numbSelected`, numSelected)
      if (numSelected == 0) pivot = item;
      if (item.selected) selected.add(item);
      else (selected.delete(item));
      numSelected = selected.size;
      console.log(`numbSelected`, numSelected, selected)
      // if (!numSelected && prevNumSelected == 1) {
      //   pivot = null;
      // }
      // if(numSelected){

      // }
      // let unselected = items.filter((v) => v != pivot);
      // unselected.forEach((item) =>
      //   console.log(sim.compareTwoStrings(item.basename, pivot.basename), fuzz.match(item.basename, pivot.basename))
      // );
      // unselected.sort((a, b) =>
      //   sim.compareTwoStrings(a.basename, pivot.basename) > sim.compareTwoStrings(b.basename, pivot.basename) ? -1 : 1
      // );
      // items = [pivot, ...unselected];
