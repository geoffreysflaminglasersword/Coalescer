<script lang="ts">
  ...
  import { App, CachedMetadata, FileSystemAdapter, getAllTags, Menu, parseFrontMatterTags, TFile, WorkspaceLeaf } from "obsidian";
  ...
  async function coalesce() {
    ...
    */
    let callback: CB, pivContents:string;
    // if (pivot instanceof Tag) {
    //   callback = (f) => {

    //   };
    // } else if (pivot instanceof File) {
    //   app.vault.getAbstractFileByPath(pivot.path);
    //   pivContents = await app.vault.adapter.read(pivot.path);
  ...
  type CB = (file: TFile,isAlsoItem:boolean) => void;
  function findAndReplaceWith(find: Item | Item[], replace: Item, callback?: CB) {
    if (!Array.isArray(find)) findAndReplaceWith([find], replace, callback);
      ...
      app.vault.getMarkdownFiles().forEach((file) => {
        let fileIsItem = find.find(item => item instanceof File  && item.path == file.path);
        if(fileIsItem)console.log(`fileIsItem`, fileIsItem)
