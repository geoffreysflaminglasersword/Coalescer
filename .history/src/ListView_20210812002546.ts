...
export default class CoalescerListView extends ItemView {
    ...
    public readonly redraw = (): void => {
        ...
        console.log(`this.app.metadataCache.unresolvedLinks`, Object.entries(this.app.metadataCache.unresolvedLinks).filter(([key,val]) => Object.entries(val)?.length).map(([key,val]) => {let obj = {}; obj[key] = val; return obj}))
        console.log(`this.app.metadataCache.resolvedLinks`, Object.entries(this.app.metadataCache.resolvedLinks).filter(([key,val]) => Object.entries(val)?.length).map(([key,val]) => {let obj = {}; obj[key] = val; return obj}))
