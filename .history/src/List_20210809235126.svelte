<script lang="ts">
  ...
  let pivot: Item;
  let lastSortResultIndex:nubmer;
      ...
      if (numSelected == 1 && prevNumSelected == 0) {
        ...
        items = [
          ...new Set<[Item, boolean]>([
            ...
            ...(pivot.basename.split(/[.\-, ;_]/).map(token => matchSorter(filt, token, { keys: ["*.basename"] })).flat(1)),
            ...(pivot.basename.split(/[.\-, ;_]/).map(token => matchSorter(filt, token.slice(0,-1), { keys: ["*.basename"] })).flat(1)),
            ...(pivot.basename.split(/[.\-, ;_]/).map(token => matchSorter(filt, token.slice(1), { keys: ["*.basename"] })).flat(1)),
            ...(pivot.basename.split(/[.\-, ;_]/).map(token => matchSorter(filt, token.slice(0,-3), { keys: ["*.basename"] })).flat(1)),
            ...(pivot.basename.split(/[.\-, ;_]/).map(token => matchSorter(filt, token.slice(3), { keys: ["*.basename"] })).flat(1)),
            ...(pivot.basename.split(/[.\-, ;_]/).map(token => matchSorter(filt, token.slice(0,-5), { keys: ["*.basename"] })).flat(1)),
            ...(pivot.basename.split(/[.\-, ;_]/).map(token => matchSorter(filt, token.slice(5), { keys: ["*.basename"] })).flat(1)),
            ...(pivot.basename.split(/[.\-, ;_]/).map(token => matchSorter(filt, token.slice(0,-8), { keys: ["*.basename"] })).flat(1)),
            ...(pivot.basename.split(/[.\-, ;_]/).map(token => matchSorter(filt, token.slice(8), { keys: ["*.basename"] })).flat(1)),
        ])];
        let lastSortResultIndex = items.length;
