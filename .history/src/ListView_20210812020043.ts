...
export default class CoalescerListView extends ItemView {
    ...
    public readonly redraw = (): void => {
        ...
        let allLinks = [...Object.entries(this.app.metadataCache.unresolvedLinks),...Object.entries(this.app.metadataCache.resolvedLinks)]
        console.log(`allLinks`, Object.entries(this.app.metadataCache.unresolvedLinks),Object.entries(this.app.metadataCache.resolvedLinks))
        allLinks.forEach(([path, links]) => {
            if(this.plugin.state.omittedPaths.some(matcher => path.match(new RegExp(matcher,'i'))))
                return;
			Object.entries(links).forEach(([name, count]) => {
                name = name.match(matchFileName,'$1')[1];
                if(name.match(/homer/i)) console.log('HERE:',path,name,count,links.get(name))
