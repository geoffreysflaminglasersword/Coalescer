  ...
  let click = (item: Item) =>
    ((event: MouseEvent) => {
      ...
      if (numSelected == 1 && prevNumSelected == 0) {
        ...
        console.log(`pivot.basename.split(/[.\-, ;_](?=[A-Z])?/)`, pivot.basename.split(/[.\-, ;_]/).concat(pivot.basename.split(/(?=[A-Z])/)));
        items = [...new Set<[Item, boolean]>([
            [pivot, true],
            ...matchSorter(filt, pivot.basename, { keys: ["*.basename"] }),
            ...(pivot.basename.split(/[.\-, ;_]/).concat(pivot.basename.split(/(?=[A-Z])/)).map(token => matchSorter(filt, token, { keys: ["*.basename"] })).flat(1)),
            ...([1,3,5,8,13].map(i => (pivot.basename.split(/[.\-, ;_]/).concat(pivot.basename.split(/(?=[A-Z])/)).map(token => Math.max(i*2,5)<=token.length?  matchSorter(filt, token.slice(0,-i), { keys: ["*.basename"] }):[]).flat(1))).flat(1)),          
            ...([1,3,5,8,13].map(i => (pivot.basename.split(/[.\-, ;_]/).concat(pivot.basename.split(/(?=[A-Z])/)).map(token => Math.max(i*2,5)<=token.length? matchSorter(filt, token.slice(i), { keys: ["*.basename"] }):[]).flat(1))).flat(1)),
        ])];
