    ...
    if (!Array.isArray(find)) findAndReplaceWith([find], replace, useAlias, preprocess);
      ...
      let replaceTagsLinksAndEmbeds = async (file: TFile, contents: string) => {
        ...
        foundAlts.forEach((alt) => {
          if (useAlias && !(alt instanceof Tag)) aliases.push(alt.basename);
      ...
      if (pivot instanceof File) replaceTagsLinksAndEmbeds(pivot.tfile, await app.vault.cachedRead(pivot.tfile));
    }
  }
