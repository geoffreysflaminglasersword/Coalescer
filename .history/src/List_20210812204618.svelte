<script lang="ts">
  ...
  import { Item, Tag, File, Link, CoalescerState, linkOpts, updateYamlProp, getYamlProp,subsets } from "./SettingsAndUtils";
  ...
  let click = (item: Item) =>
    ((event: MouseEvent) => {
      if (numSelected == 1) {
        pivot = items.find(val => val.selected);
        let filt = items.filter(v => v != pivot);
        //this is an inneficient (and disgusting) way to get better suggestions, but it works for now
        let tokens = subsets(pivot.basename.split(/[.\-, ;_]/)).concat(subsets(pivot.basename.split(/(?=[A-Z])/)));
        console.log(`tokens`, filt, pivot.basename, fuzz.filter(filt,pivot.basename,{key:'basename'}),fuzz.filter(filt,'merge',{key:'basename'}))
        filt.forEach(item => console.log(sim.compareTwoStrings(item.basename,pivot.basename),fuzz.match(item.basename,pivot.basename)))
        filt.sort((a,b) => sim.compareTwoStrings(a.basename,pivot.basename) > sim.compareTwoStrings(b.basename,pivot.basename) ? -1:1)
        items = [
          ...new Set<Item>([
            pivot,
            // ...matchSorter(filt, pivot.basename, { keys: ["*.basename"] }),
            // ...fuzz.filter(filt,pivot.basename,{key:'basename'})
          ]),
        ].slice(0, 30); //limit results
        
        for(let subset of tokens){
          // console.log(` subset.join(' ')`,  subset.join(' '));
          // console.log(`fuzz.filter(filt,subset.join(' '),{key:'basename'})`, fuzz.filter(filt,subset.join(' '),{key:'basename'}))
          // items = [...items,...fuzz.filter(filt,subset.join(' '),{key:'basename'})];
          // [1, 3, 5, 8, 13]
          //     .map((i) =>
