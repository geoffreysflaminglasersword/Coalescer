      ...
      if (numSelected == 1 && prevNumSelected == 0) {
        ...
        items = [
          ...new Set<[Item, boolean]>([
            ...
            ...pivot.basename
              .split(/[.\-, ;_]/)
              .concat(pivot.basename.split(/(?=[A-Z])/))
              .map((token) => matchSorter(filt, token, { keys: ["*.basename"] }))
              .flat(1),
            ...[1, 3, 5, 8, 13]
              .map((i) =>
                pivot.basename
...
<div class="listToggles">
  <div class='toggleText'>
    ...
    <div class="checkbox-container {showFiles?'is-enabled':''}" />
  <div class='toggleText'>
    Show Tags
  </div>
    <div class="checkbox-container {showTags?'is-enabled':''}" />
</div>
