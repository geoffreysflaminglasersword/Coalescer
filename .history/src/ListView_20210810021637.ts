...
export default class CoalescerListView extends ItemView {
    ...
    public readonly redraw = (): void => {
        ...
        let omit = this.plugin.state.omittedPaths;
        let files = this.app.vault.getMarkdownFiles();
        console.log(`omit.length`, omit.length)
        if(omit.length) files = files.filter(f => this.plugin.state.omittedPaths.every(p => !f.path.match(p)));
        files.map<[Item,boolean]>(v => [v,false]).forEach(v => this.items.push(v));
