    ...
    ((event: MouseEvent) => {
      if (numSelected == 1 && prevNumSelected == 0) {
        ...
        items = [...new Set<[Item, boolean]>([
            ...
            ...([1,3,5,8].map(i => (pivot.basename.split(/[.\-, ;_]/).map(token => i*2<=token.length? matchSorter(filt, token.slice(i), { keys: ["*.basename"] }):[]).flat(1))).flat(1)),
            ...([1,3,5,8].map(i => (pivot.basename.split(/[.\-, ;_]/).map(token => i*2<=token.length?  matchSorter(filt, token.slice(0,-i), { keys: ["*.basename"] }):[]).flat(1))).flat(1)),          
        ])];
