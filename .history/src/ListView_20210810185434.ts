...
export default class CoalescerListView extends ItemView {
    ...
    public readonly redraw = (): void => {
        ...
        Object.entries(this.app.metadataCache.unresolvedLinks).forEach(([key, value]) => 
			Object.entries(value).forEach(([key, value]) => filt.push([key,value]))
		);
        filt = [...new Set(filt)];
        ...
        filt.forEach(([key,value]) => {
            ...
            if(!this.plugin.state.omittedPaths.every(matcher => {
                console.log(`!key.match(new RegExp(matcher,'i'))`, !key.match(new RegExp(matcher,'i')), '  :  ',matcher,'  :  ', key)
                return !key.match(new RegExp(matcher,'i'))
            })) return;
			Object.entries(value).forEach(([k, v]) => this.items.push([new UnresolvedLink(k,v),false]));
		});
