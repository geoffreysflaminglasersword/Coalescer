<script lang="ts">
  ...
  import { HasCount, Item, Tag, File, Link } from "./Settings";
  ...
  const linkOpts = ["All", "Resolved", "Unresolved"] as const;
  type LinkOpt = TFromArray<typeof linkOpts>;
  let linkView: LinkOpt;
  ...
  function findAndReplaceWith(find: Item | Item[], replace: Item) {
    if (!Array.isArray(find)) findAndReplaceWith([find], replace);
      ...
      app.vault.getMarkdownFiles().forEach((file) => {
        ...
        if(cache.links) console.log(`cache.links`, cache.links)
  ...
  let shouldShow = (item: Item) => {
    ...
    if (item instanceof File) return  pivot ? (pivot instanceof File ? showFiles : false) : showFiles;
    if (item instanceof Link)
      return (
        linkView == "All" || (linkView == "Resolved" && item.resolved) || (linkView == "Unresolved" && !item.resolved)
      );
  };
...
<div>
  <div class="container">
    <div class="listToggles">
      ...
      <select class="dropdown" bind:value={linkView} on:change={() => (items = items)}>
