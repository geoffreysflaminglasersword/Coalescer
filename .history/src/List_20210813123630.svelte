    ...
    ((event: MouseEvent) => {
      ...
      if (numSelected) {
        ...
        selected.forEach(item => {
          ...
          let similar = unselected.sort((a, b) => 
            sim.compareTwoStrings(a.basename, pivot.basename) > sim.compareTwoStrings(b.basename, pivot.basename) ? -1 : 1).slice(15);
          items = [...similar,...items];
        })
        items = [...selected,...new Set(items)]; //the above step can introduce duplicates
      }
